function [cov_all, cov_sum] = deepmain()
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This method deepmain returns and saves the SPD matrices
% 1. Covariance matrix
% 2. Sum of Covariance Maps
%
% This method also stores the SPD matrices in respective .mat files.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
clear;
clc;
% author: Rui Wang
...
try chol(cov_sum);
    disp('Matrix cov_sum is symmetric positive definite.')
    save("cov_sum_deepmain_v1_SymNet.mat", "cov_sum")
catch ME
    disp('Matrix cov_sum is not symmetric positive definite')
end 
...
try chol(cov_all);
    ...
    save("cov_all_deepmain_v1_SymNet.mat", "cov_all")
catch ME
...
for i = 1:size(Train_labels,2)
    ...
    for m = 1:num_layers
        % Get the SPD layer to dump the data file
        temp = second_layer_singletr{m};
        % Matrix temp is not symmetric positive definite 
        [u,v,w] = svd(temp);
        ...
        % Matrix pooling is not symmetric positive definite
        [u,v,w] = svd(pooling);
        logv = exp(diag(v));
    ...
    for p=1:num_layers
        ...
        % Matrix log_map{p} is not symmetric positive definite 
        [u,v,w] = svd(log_map{p});
        logv = log(diag(v));
    ...
    % output layer
    for n=1:num_layers
        % Matrix log_map{n} is not symmetric positive definite 
        [U,V,D] = svd(log_map{n});
        all_trace(n) = trace(V);
    end
    for n = 1:num_layers
...
for i = 1:size(Test_labels,2)
    ...
    for k = 1:num_layers
        ...
        idx = idx1 & idx2;
        mid(idx) = -eta_1;
        % Matrix mid is not symmetric positive definite
        [U,V,D] = svd(mid);
        [a,b] = size(V);
        tol = trace(V) * eps_1;  
        for l=1:a
...
fprintf(1,'\nthe right classified test samples are: %d\n',accuracy_number);
