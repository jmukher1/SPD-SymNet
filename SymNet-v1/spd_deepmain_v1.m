function [cov_train, cov_all, cov_test, cov_sum] = spd_deepmain_v1()
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This method spd_deepmain_v1 checks all the intermediate 
% matrices are Symmetric Positive Definite (SPD) and returns the
% SPD matrices
% 1. Covariance matrix
% 2. Sum of Covariance Maps
% For saving the covariance (SPD) matrices, please run deepmain.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
clear;
clc;
% author: Rui Wang
...
for i = 1:size(Train_labels,2)
    ...
    for m = 1:num_layers
        ...
        % Matrix pooling 124 is not symmetric positive definite
        [u,v,w] = svd(pooling);
        logv = exp(diag(v));
        mean_pool{m} = u * diag(logv) * w';
    end
    
    % Matrix mean_pool is not symmetric positive definite
    ...
    % Matrix log_map is not symmetric positive definite 
...
for i = 1:size(Test_labels,2)
    ...
    % Matrix second_layer_singlete is not symmetric positive definite 

    % SPD matrix pooling layer: tangent space pooling tactic
    for m=1:num_layers
        temp = second_layer_singlete{m};
        % Matrix temp is symmetric positive definite. 
        [u,v,w] = svd(temp);
        logv = log(diag(v));
        temp = u * diag(logv) * w';
...
gnd = Train_labels(1,:); 
options.Regu = 1;
[eigvector, eigvalue, B, T] = fun_SymNet_Train(options, gnd, kmatrix_train);
nClasses = unique(gnd);
